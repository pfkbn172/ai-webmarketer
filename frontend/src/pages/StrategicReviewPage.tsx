...
function ReviewBox() {
  const review = useMutation<StrategicReview, Error>({ mutationFn: runReview });
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-3">
        <Button onClick={() => review.mutate()} disabled={review.isPending}>
          {review.isPending ? '生成中…(30〜60 秒)' : '今すぐ戦略レビュー'}
        </Button>
        {review.data && (
          <div className="space-y-3 text-sm">
            <Section title="構造的な発見" items={review.data.core_findings ?? []} />
            <Section
              title="事業文脈と整合している点"
              items={review.data.alignments_with_business_context ?? []}
            />
            <Section title="ずれている兆候" items={review.data.misalignments ?? []} />
            {(review.data.next_actions ?? []).length > 0 && (
              <div>
                <h4 className="font-semibold">推奨アクション</h4>
                <ol className="mt-1 list-decimal space-y-2 pl-5">
                  {review.data.next_actions!.map((a, i) => (
                    <li key={i}>
                      <div>{a.action}</div>
...
function ProbeLoopBox() {
  const probe = useMutation<ProbeLoopResult, Error>({ mutationFn: runProbeLoop });
  return (
    <Card>
      <CardHeader>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          ...
          AI に推論比較させ、勝者と推奨クエリを返します。
        </p>
      </CardHeader>
      <CardContent className="space-y-3">
        <Button onClick={() => probe.mutate()} disabled={probe.isPending}>
          {probe.isPending ? '比較中…' : '比較を実行'}
        </Button>
        {probe.data && (
          <div className="grid gap-3 md:grid-cols-2 text-sm">
            <AxisCard label="軸 A" axis={probe.data.axis_a} winner={probe.data.winner === 'axis_a'} />
            <AxisCard label="軸 B" axis={probe.data.axis_b} winner={probe.data.winner === 'axis_b'} />
            {probe.data.winner_rationale && (
              <div className="md:col-span-2 rounded-md border border-border bg-muted p-3">
                <div className="font-semibold">勝者の理由</div>
                <p className="mt-1 text-muted-foreground">{probe.data.winner_rationale}</p>
              </div>
            )}
          </div>
        )}
