"""GA4 ai_crawler_visit イベントの日次集計(クローラー名別)。"""
...
class Ga4AiCrawlerDaily(Base, IdMixin, TenantMixin, TimestampsMixin):
    __tablename__ = "ga4_ai_crawler_daily"
    __table_args__ = (
        UniqueConstraint(
            "tenant_id", "date", "crawler_name", name="uq_ga4_ai_crawl_tenant_date_name"
        ),
        Index("ix_ga4_ai_crawl_tenant_date", "tenant_id", "date"),
    )
