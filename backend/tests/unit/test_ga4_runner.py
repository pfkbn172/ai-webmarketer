...
async def test_upsert_called_with_rows(
    ...
    fake_client.hourly_metrics = AsyncMock(return_value=[])
    fake_client.referrals_daily = AsyncMock(return_value=[])
    fake_client.referrals_hourly = AsyncMock(return_value=[])
    fake_client.ai_referral_events = AsyncMock(return_value=[])
    fake_client.ai_crawler_visits = AsyncMock(return_value=[])
    fake_client.ai_crawler_pages = AsyncMock(return_value=[])
    fake_client.llms_txt_fetches = AsyncMock(return_value=[])
    fake_client.article_read_completes = AsyncMock(return_value=[])
    fake_client.text_copy_events = AsyncMock(return_value=[])
    fake_client.outbound_click_events = AsyncMock(return_value=[])
    fake_client.cta_click_events = AsyncMock(return_value=[])
    fake_client.tool_use_events = AsyncMock(return_value=[])
    fake_client.engagement_signals = AsyncMock(return_value=[])
